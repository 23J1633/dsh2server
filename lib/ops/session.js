...
export function createSessionOps(deps) {
  ...
  return {
    ...
    'session.get': async (params) => {
      ...
      let detail = host.sessionDetail(sessionId)
      let exists = detail.attached || detail.header !== undefined
      if (!exists) {
        ...
        detail = host.sessionDetail(sessionId)
        exists = detail.attached || detail.header !== undefined
      }
    ...
    /** Hide a session from the host's workspace grouping surfaces. */
    'session.archive': async (params) => {
      const sessionId = requireString(params, 'sessionId', { maxLength: 200 })
      assertSessionAllowed(sessionId)
      const result = await host.archiveSession(sessionId)
      deps.publish({
        topic: 'sessions',
        kind: 'session/archived',
        sessionId,
        data: { sessionId, at: Date.now() },
      })
