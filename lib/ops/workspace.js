...
export function createWorkspaceOps(deps) {
  ...
  return {
    ...
    /** List safe starting points for the remote directory picker. */
    'workspace.fs.roots': async () => await host.workspaceRoots(),

    /** Create a child directory in the currently browsed directory. */
    'workspace.fs.mkdir': async (params) => {
      const path = requireString(params, 'path', { maxLength: 4096 })
      const name = requireString(params, 'name', { maxLength: 255 })
      return await host.createDirectory({ path, name })
    },
