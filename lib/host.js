...
export class HostAdapter {
  ...
  constructor(options) {
    ...
    this.terminals.on('output', (data) => this.publish({ topic: 'terminal', kind: 'terminal/output', data }))
    this.terminals.on('exit', (data) => this.publish({ topic: 'terminal', kind: 'terminal/exit', data }))
    /** @type {Map<string, unknown>} */
    this.cache = new Map()
    /** @type {Map<string, Record<string, any>>} last controller summaries, including cold sessions. */
    this.sessionSummaries = new Map()
    /** @type {Map<string, Record<string, any>>} per-module display facts, so `plugin.list` imports each once. */
    this.pluginFacts = new Map()
    /**
  ...
  capabilities() {
    ...
    return {
      ...
      sessionArchive: typeof this.service('workspaceController')?.archiveSession === 'function'
        || typeof this.service('workspaceRegistry')?.archiveSession === 'function',
      ...
      agentPresets: present.agentPresets,
      ...
      terminal: this.config.allowRemoteTerminal === true,
    }
  }
  ...
  async listSessions(signal) {
    ...
    if (controller?.list) {
      ...
      this.#rememberSessionSummaries(rows)
      return rows
    }
    const rows = new Map()
    const sessions = this.service('sessions')
    ...
    const visible = [...rows.values()].filter((row) => this.allowsCwd(row.cwd)).sort((a, b) => b.updatedAt - a.updatedAt)
    this.#rememberSessionSummaries(visible)
    return visible
  }
  ...
  terminalOpen(params = {}) {
    ...
    this.#assertPathAllowed(cwd)
    return this.terminals.open({ ...params, cwd })
  }

  terminalList() { this.#assertTerminalEnabled(); return { items: this.terminals.list() } }
  terminalAttach(terminalId) { this.#assertTerminalEnabled(); return this.terminals.attach(terminalId) }
  terminalKeepAlive(terminalId) { this.#assertTerminalEnabled(); return this.terminals.keepAlive(terminalId) }
  terminalWrite(terminalId, data) { this.#assertTerminalEnabled(); return this.terminals.write(terminalId, data) }
  terminalResize(terminalId, cols, rows) { this.#assertTerminalEnabled(); return this.terminals.resize(terminalId, cols, rows) }
  terminalClose(terminalId) { this.#assertTerminalEnabled(); return this.terminals.close(terminalId) }

  #assertTerminalEnabled() {
    if (!this.config.allowRemoteTerminal) throw fail(ERROR_CODES.DISABLED, 'remote terminal is disabled by plugin configuration')
  }

  /**
   * Keep the controller's cold-session facts available to synchronous
   * operation guards and `session.get` without activating the session.
   *
   * @param {Array<Record<string, any>>} rows normalized session summaries.
   */
  #rememberSessionSummaries(rows) {
    this.sessionSummaries.clear()
    for (const row of rows) this.sessionSummaries.set(String(row.sessionId), row)
  }
  ...
  sessionDetail(sessionId) {
    ...
    const summaryModel = summary?.projections?.values?.modelSelection?.next
      ?? summary?.projections?.values?.modelSelection?.lastUsed
    const summaryInbox = summary?.projections?.values?.inbox
    ...
    const detail = {
      ...
      header: summary === undefined ? undefined : {
        id: sessionId,
        cwd: summary.cwd,
        parentSession: summary.parentSessionId,
        origin: summary.origin,
        agentPreset: summary.agentPreset ?? summary.projections?.values?.agentPreset,
      },
      seq: summary?.projections?.asOfSeq,
      projections: summary?.projections,
      model: summaryModel,
      pending: summaryInbox === undefined ? undefined : {
        nextTurn: Array.isArray(summaryInbox['next-turn']) ? summaryInbox['next-turn'].length : 0,
        nextStep: Array.isArray(summaryInbox['next-step']) ? summaryInbox['next-step'].length : 0,
      },
    }
    if (session) {
   ...
   * @param {number} [toSeqExclusive] exclusive last sequence number; omitted reads to the end.
   * @param {AbortSignal} [signal] caller lifetime.
  ...
  async readLog(sessionId, fromSeq, toSeqExclusive, signal) {
    ...
    if (persistence?.open) {
      ...
      try {
        const length = toSeqExclusive === undefined ? undefined : Math.max(0, toSeqExclusive - fromSeq)
        const result = await handle.read(fromSeq, length, signal === undefined ? {} : { signal })
    ...
    if (controller?.inspect) {
      ...
      return events.filter(
        (event) =>
          Number(event?.seq) >= fromSeq && (toSeqExclusive === undefined || Number(event?.seq) < toSeqExclusive),
      )
    }
  ...
  /**
   * Read every rating in one session, keyed by the log position a server sees.
   *
   * The harness stores feedback against the message's own durable id, which no
   * server ever sees; the message-aligned log is what maps one to the other.
   *
   * @param {string} sessionId target session.
   * @param {AbortSignal} [signal] caller lifetime.
   * @returns {Promise<Array<Record<string, unknown>>>} `{ seq, rating, updatedAt }` rows in log order.
   */
  async listFeedbackBySeq(sessionId, signal) {
    const items = await this.listFeedback(sessionId)
    if (items.length === 0) return []
    const byMessage = new Map(items.map((item) => [String(item.messageId), item]))
    const events = await this.readLog(sessionId, 0, undefined, signal)
    /** @type {Array<Record<string, unknown>>} */
    const rows = []
    for (const event of events) {
      if (event?.type !== 'assistant/message') continue
      const item = byMessage.get(String(event?.data?.message?.id ?? ''))
      if (item === undefined) continue
      rows.push({ seq: Number(event.seq), rating: feedbackRating(item.rating), updatedAt: Number(item.updatedAt ?? 0) })
    }
    return rows
  }
   ...
   *
   * A live session answers from its own state. A cold one is read from its log
   * instead of being activated: the harness derives the preset from the last
   * `permission/preset`, `sandbox/mode`, and `approval/policy` events, and the
   * service exposes that same fold, so a console can render the selector for a
   * session nobody has opened yet.
   *
   * @param {string} sessionId target session.
   * @param {AbortSignal} [signal] caller lifetime.
   * @returns {Promise<Record<string, unknown> | undefined>} the preset view.
   */
  async permissionOf(sessionId, signal) {
    const service = this.service('permissionPresets')
    if (!service?.names) return undefined
    const session = this.liveSession(sessionId)
    if (session !== undefined && typeof service.current === 'function') {
      const current = safeCall(() => service.current(session))
      return typeof current === 'string' ? this.#permissionView(service, current) : undefined
    }
    if (typeof service.selectFor !== 'function') return undefined
    const state = await this.#permissionKnobs(sessionId, signal)
    const select = safeCall(() => service.selectFor(state))
    return typeof select?.currentValue === 'string' ? this.#permissionView(service, select.currentValue) : undefined
  }

  /**
   * Fold the three whole-value permission knobs out of one session's log.
   *
   * @param {string} sessionId target session.
   * @param {AbortSignal} [signal] caller lifetime.
   * @returns {Promise<{preset: string|null, sandbox: string|null, approval: string|null}>} the folded state.
   */
  async #permissionKnobs(sessionId, signal) {
    const events = await this.readLog(sessionId, 0, undefined, signal)
    /** @type {{preset: string|null, sandbox: string|null, approval: string|null}} */
    const state = { preset: null, sandbox: null, approval: null }
    for (const event of events) {
      if (event?.type === 'permission/preset') state.preset = event?.data?.preset ?? null
      else if (event?.type === 'sandbox/mode') state.sandbox = event?.data?.mode ?? null
      else if (event?.type === 'approval/policy') state.approval = event?.data?.policy ?? null
    }
    return state
  }

  /**
   * Build the protocol view of one permission preset selection.
   *
   ...
   * only reported when the deployment actually declares one, so the server can
   * otherwise fall back to its built-in copy.
   *
   * @param {any} service the permission-preset service.
   * @param {string} current the deployment's effective preset name.
   * @returns {Record<string, unknown>} `{ preset, available, labels? }`.
   */
  #permissionView(service, current) {
    ...
    for (const name of names) {
      ...
      available.push(id)
      const declared = safeCall(() => service.resolve(name)?.name)
      if (typeof declared === 'string' && declared !== '') labels[id] = declared
    }
    return {
  ...
  async setPermission(sessionId, preset) {
    ...
    return this.#permissionView(service, String(safeCall(() => service.current(session)) ?? name))
  }

  /**
   * Archive one session through DSH's supported workspace registry API.
   * This removes it from local grouping surfaces but intentionally preserves
   * the durable log, matching the official client behavior.
   *
   * @param {string} sessionId target session.
   * @returns {Promise<Record<string, unknown>>} archive acknowledgement.
   */
  async archiveSession(sessionId) {
    const controller = this.service('workspaceController')
    if (typeof controller?.archiveSession === 'function') {
      const value = await controller.archiveSession({ sessionId })
      return { archived: true, archivedSessionIds: value?.archivedSessionIds ?? [] }
    }
    const registry = this.service('workspaceRegistry')
    if (typeof registry?.archiveSession !== 'function') {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'session archive requires the workspace registry plugin')
    }
    await registry.archiveSession(sessionId)
    return { archived: true, archivedSessionIds: [...(registry.archivedSessionIds ?? [])] }
  }

  // ── Agent presets (the local UI's per-session composition picker) ─────────

  /**
   * Return the path-free Agent-preset roster used by the local web UI.
   *
   * @returns {Promise<{presets: Array<Record<string, unknown>>, authorable: boolean, modeSelectionEnabled: boolean}>}
   */
  async listAgentPresets() {
    const service = this.service('agentPresets')
    if (!service) {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'Agent presets require the agent-presets plugin')
    }
    if (typeof service.remoteExportList === 'function') {
      ...
      return {
        ...
        modeSelectionEnabled: roster?.modeSelectionEnabled !== false,
      }
    }
    if (typeof service.list !== 'function') {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'this Agent-preset service cannot list its roster')
    }
    const presets = await service.list()
    const defaultId = String(service.defaultId ?? '')
    return {
      // Do not expose the local preset directory path across the relay.
      presets: (Array.isArray(presets) ? presets : []).map((preset) => ({
        id: String(preset?.id ?? ''),
        trust: preset?.trust,
        isDefault: String(preset?.id ?? '') === defaultId,
  ...
  /** Read one preset composition without exposing its directory. */
  async readAgentPreset(agentPreset) {
    const service = this.service('agentPresets')
    if (!service) {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'Agent presets require the agent-presets plugin')
    }
    if (typeof service.readDocument === 'function') return await service.readDocument(agentPreset)
    if (typeof service.read === 'function') {
      return { agentPreset, content: await service.read(agentPreset) }
    }
    throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'this Agent-preset service cannot read compositions')
  }

  /** Select one preset for a blank session. DSH itself enforces the blank lock. */
  async selectAgentPreset(sessionId, agentPreset) {
    ...
    if (cwd !== undefined && !this.allowsCwd(cwd)) {
    ...
    const service = this.service('agentPresets')
    if (!service?.select) {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'Agent-preset selection requires the agent-presets plugin')
    }
    const agent = await this.ensureAgent(sessionId)
    const selected = await service.select(agent, agentPreset)
    return { agentPreset: String(selected ?? agentPreset) }
  }

  /** Copy a preset into the writable user roster. */
  async copyAgentPreset(params) {
    if (!this.config.allowRemoteControl) {
      throw fail(ERROR_CODES.DISABLED, 'remote control is disabled by plugin configuration')
    }
    const service = this.service('agentPresets')
    ...
    if (typeof service.remoteExportCopy === 'function') {
      await service.remoteExportCopy(params.from, params.agentPreset, params.name)
    } else if (typeof service.copy === 'function') {
      await service.copy(params.from, params.agentPreset, params.name)
    } else {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'this Agent-preset service cannot copy compositions')
    }
    return { accepted: true, agentPreset: params.agentPreset }
  }

  /** Delete one user-authored preset. Built-ins are refused by the host. */
  async deleteAgentPreset(agentPreset) {
    if (!this.config.allowRemoteControl) {
      throw fail(ERROR_CODES.DISABLED, 'remote control is disabled by plugin configuration')
    }
    const service = this.service('agentPresets')
    ...
    if (typeof service.remoteExportDelete === 'function') {
      await service.remoteExportDelete(agentPreset)
    } else if (typeof service.remove === 'function') {
      await service.remove(agentPreset)
    } else {
      throw fail(ERROR_CODES.CAPABILITY_UNAVAILABLE, 'this Agent-preset service cannot delete compositions')
    }
    return { accepted: true }
  }
  ...
  async renameWorkspace(params) {
    ...
    const workspaceId = await this.findWorkspace(params.id, params.path)
    const value = await controller.rename({ workspaceId, title: params.title })
    return { workspace: wireWorkspace(value?.workspace) }
  }
   ...
   * @returns {Promise<string>} the matching registered workspace's id.
   */
  async findWorkspace(id, path) {
    ...
    const match =
      id !== undefined
        ? workspaces.find((entry) => String(workspaceIdOf(entry)) === String(id))
        : workspaces.find((entry) => samePath(entry?.path, path))
    if (match === undefined) {
      throw fail(
        ERROR_CODES.NOT_FOUND,
        id !== undefined ? `unknown workspace "${id}"` : `no workspace is registered for "${path}"`,
        { details: id !== undefined ? { id } : { path } },
      )
    }
    return String(workspaceIdOf(match))
  }
  ...
  async readFile(params) {
    ...
    // Binary previews are cut with a byte window: asking the backend for a
    // whole-file read under a smaller cap is refused outright (the cap applies
    // to the file, not to the window), which would turn every large image into
    // an error instead of a preview.
    if (budget === undefined) {
      const bytes = await fs.readBytes(target, undefined, Math.max(size, 1))
      return { path: params.path, size, truncated: false, binary: true, dataBase64: Buffer.from(bytes).toString('base64'), mime: sniffMime(params.path) }
    }
    const window = await fs.readByteRange(target, { offset: 0, length: budget })
    return {
      path: params.path,
      size,
      truncated,
      binary: true,
      dataBase64: Buffer.from(window).toString('base64'),
      mime: sniffMime(params.path),
    }
  }

  /** Roots exposed to the remote directory picker. */
  async workspaceRoots() {
    const configured = this.config.allowedCwdPrefixes ?? []
    const candidates = configured.length
      ? configured
      : process.platform === 'win32'
        ? 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map((letter) => `${letter}:\\`).filter((path) => existsSync(path))
        : ['/']
    const roots = []
    for (const candidate of candidates) {
      const path = await realpath(candidate).catch(() => null)
      if (!path || roots.some((entry) => samePath(entry.path, path))) continue
      roots.push({ path, name: parse(path).root === path ? path : (parse(path).base || path) })
    }
    return { roots }
  }

  /** Create one direct child of an allowed directory for the remote picker. */
  async createDirectory(params) {
    const parent = await realpath(params.path)
    this.#assertPathAllowed(parent)
    const name = String(params.name ?? '').trim()
    if (!name || name === '.' || name === '..' || /[\\/]/.test(name)) {
      throw fail(ERROR_CODES.INVALID_PARAMS, 'folder name must be one path segment', { details: { field: 'name' } })
    }
    const path = join(parent, name)
    this.#assertPathAllowed(path)
    await mkdir(path)
    const info = await stat(path)
    if (!info.isDirectory()) throw fail(ERROR_CODES.INTERNAL, `created path is not a directory: ${path}`)
    return { path: await realpath(path), name }
  }
  ...
  async listPlugins() {
    ...
    for (const entry of snapshot?.entries ?? []) {
      rows.push({ id: String(entry?.entryId ?? ''), moduleName: String(entry?.moduleName ?? ''), scope: 'global', enabled: entry?.enabled === true, phase: entry?.fiberPhase })
    }
    ...
    // Facts are resolved in parallel: the first `list` imports every distinct
    // module once, and later calls are cache hits.
    const items = await Promise.all(
      rows.map(async (row) => {
        const facts = await this.#pluginFacts(row.moduleName)
        return {
          id: row.id,
          name: facts.name,
          ...
          enabled: row.enabled,
          state: fiberState(row.phase, row.enabled),
          scope: row.scope,
          ...(row.preset === undefined ? {} : { preset: row.preset }),
          configurable: row.scope === 'global' && facts.configurable,
        }
      }),
    )
    return { items }
  ...
  async pluginConfig(id) {
    ...
    const schema = await this.#loaderSchema(String(entry?.options?.name ?? ''))
    return {
      schema,
   ...
   * @returns {Promise<Record<string, unknown>>} the module's `Config` as JSON Schema, or an empty object.
   */
  async #loaderSchema(moduleName) {
    const module = await this.#loaderModule(moduleName)
    const config = moduleConfig(module)
    if (typeof config?.toJSON !== 'function') return {}
    return toPlainJson(schemaToJsonSchema(safeCall(() => config.toJSON()))) ?? {}
  }
  ...
  async setPluginEnabled(id, enabled) {
    ...
    const facts = await this.#pluginFacts(moduleName)
    return {
      item: {
        id,
        name: facts.name,
   ...
   *
   * @param {string} moduleName module specifier from the Loader entry.
   * @returns {Promise<any>} the imported module, or undefined.
   */
  async #loaderModule(moduleName) {
    if (moduleName === '') return undefined
    const loader = this.service('loader')
    if (typeof loader?.import !== 'function') return undefined
    try {
      return await loader.import(moduleName)
    } catch {
      return undefined
    }
  }
   ...
   *
   * @param {string} moduleName module specifier from the Loader entry.
   * @returns {Promise<{name: string, version: string, description?: string, configurable: boolean}>} display facts.
   */
  async #pluginFacts(moduleName) {
    const cached = this.pluginFacts.get(moduleName)
    ...
    const facts = {
      ...
      ...(typeof manifest?.description === 'string' && manifest.description !== ''
        ...
        : {}),
      configurable: moduleConfig(module) !== undefined,
    }
    this.pluginFacts.set(moduleName, facts)
    return facts
  }
...
function withFeedback(wire, source, feedback) {
  ...
  return {
    ...wire,
    feedback: { rating: feedbackRating(item.rating), updatedAt: Number(item.updatedAt ?? 0) },
  }
}

/**
 * Translate the harness's stored polarity into the protocol's vocabulary.
 *
 * @param {unknown} rating the harness's `positive` | `negative`.
 * @returns {string} `like` or `dislike`.
 */
function feedbackRating(rating) {
  return rating === 'positive' ? 'like' : 'dislike'
}
...
/**
 * @param {any} workspace a workspace row.
 * @returns {unknown} its identity, under whichever name this layer publishes.
 */
...
/**
 * Reduce a schemastery schema envelope (`Schema#toJSON()`) to the small JSON
 * Schema subset a form renderer consumes.
 *
 * Schemastery serializes to a shared ref table rather than to JSON Schema, so
 * the shapes a configuration form actually needs — nested objects, arrays,
 * scalars, the `const`-union that spells an enum, and the `title` /
 * `description` / `default` metadata — are translated here. Anything outside
 * that subset degrades to a bare node rather than to a wrong form field.
 *
 * @param {unknown} envelope a serialized schemastery schema.
 * @returns {Record<string, unknown>} the JSON Schema subset.
 */
function schemaToJsonSchema(envelope) {
  const refs = /** @type {any} */ (envelope)?.refs
  if (refs === null || typeof refs !== 'object') return {}
  const root = /** @type {any} */ (envelope)?.uid

  /**
   * @param {unknown} ref a ref-table key.
   * @param {number} depth remaining recursion budget.
   * @returns {Record<string, unknown>} the converted node.
   */
  const convert = (ref, depth) => {
    const node = /** @type {any} */ (refs)[String(ref)]
    ...
    switch (node.type) {
      case 'object': {
        /** @type {Record<string, unknown>} */
        const properties = {}
        for (const [name, child] of Object.entries(node.dict ?? {})) properties[name] = convert(child, depth + 1)
        return { ...out, type: 'object', properties }
      }
      case 'array':
        return { ...out, type: 'array', items: convert(node.inner, depth + 1) }
      case 'string':
      ...
      case 'const':
        return { ...out, enum: [node.value] }
      case 'union': {
        const values = []
        for (const child of node.list ?? []) {
          const entry = /** @type {any} */ (refs)[String(child)]
          if (entry?.type !== 'const') return out
          values.push(entry.value)
        }
        return values.length === 0 ? out : { ...out, enum: values }
      }
      default:
        // `dict`, `intersect`, `transform`, and plugin-contributed types have
        // no flat form; a renderer shows the label and leaves the value alone.
        return out
    }
  }

  return root === undefined ? {} : convert(root, 0)
}
...
/**
 * Read the configuration schema a plugin module exports.
 *
 * Cordis reads a plugin's `Config` from the module itself or, for a package
 * whose default export is the plugin function or class, from that default — so
 * both spellings are checked here.
 *
 * @param {any} module an imported plugin module namespace.
 * @returns {any} the exported schema, or undefined.
 */
