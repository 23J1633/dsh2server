...
/**
 * Lift the one-sentence intent the model writes as a tool argument.
 *
 * Every long-running tool in this harness (`pwsh`, `bash`, …) takes a
 * `description` argument and documents it as UI-facing, so the human-readable
 * half of a tool row is already inside the call's raw argument string — it only
 * needs pulling out at the wire boundary.
 *
 * @param {unknown} rawArguments the model's raw JSON argument string.
 * @returns {string | undefined} the declared intent, when there is one.
 */
export function toolIntentFromArguments(rawArguments) {
  if (typeof rawArguments !== 'string' || rawArguments === '') return undefined
  try {
    const description = JSON.parse(rawArguments)?.description
